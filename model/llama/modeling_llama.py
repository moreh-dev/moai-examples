...
if MorehRMSNorm is not None:
    logger.warning("You can't use Masked Structured Growth Training..! You should avoid using rmsnorm in any way. ")
...
ALL_LAYERNORM_LAYERS.append(LlamaRMSNorm if MorehRMSNorm is None else MorehRMSNorm)
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1, fused_rope=False):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            if self.mask is not None:
                down_proj = self.mask.to(down_proj.device).to(down_proj.dtype) * down_proj
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if ScaledDotProductAttention is not None and self.training:
            ...
            if self.mask is not None:
                attn_output = self.mask.to(attn_output.device).to(attn_output.dtype) * attn_output
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        RMSNorm = MorehRMSNorm if MorehRMSNorm is not None else LlamaRMSNorm
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if 0 < self.growth_layers <= idx:
                self.mask = self.mask.to(hidden_states.device).to(hidden_states.dtype)
                hidden_states = (1 - self.mask) * hidden_states + (self.mask) * residual
