...
ALL_LAYERNORM_LAYERS.append(InternLM2RMSNorm if MorehRMSNorm is
                            None else MorehRMSNorm)
...
class InternLM2Attention(nn.Module):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = self.q(hidden_states)  # b q (h gs d)
            key_states = self.k(hidden_states)  # b q (h d)
            value_states = self.v(hidden_states)  # b q (h d)
        """
        ...
        """
        # b q (h gs d) -> b q (h gs) d -> b (h gs) q d
        ...
        if MorehFlashAttention is None:
            ...
            if attn_output.size() != (bsz, self.num_heads, q_len,
                                      self.head_dim):
                raise ValueError(
...
class InternLM2DecoderLayer(nn.Module):
    ...
    def __init__(self, config: InternLM2Config, layer_idx: int):
        ...
        RMSNorm = InternLM2RMSNorm if MorehRMSNorm is None else MorehRMSNorm
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if sequence_length != 1:
                if support_bf16_triu or dtype == torch.float32:
                    ...
                    triu_mask = torch.triu(torch.ones(causal_mask.size(),
                                                      device=device),
                                           diagonal=1).bool()
                ...
                causal_mask[:, :, :, :
                            mask_length] = causal_mask[:, :, :, :
                                                       mask_length].masked_fill(
                                                           padding_mask,
                                                           min_dtype)
        if (self.config.attn_implementation == "sdpa" and
                attention_mask is not None and
                attention_mask.device.type == "cuda" and not output_attentions):
...
class InternLM2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            output_slices = self.output.weight.split(self.vocab_size //
                                                     self.config.pretraining_tp,
                                                     dim=0)
            logits = [
